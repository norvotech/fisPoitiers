...
class Bdd {
	...
	private static $connexion_formation = NULL;
	...
	public static function connectFormation() {

		if(!self::$connexion) {

			self::$connexion = new PDO(DNS_2, USER, PASS);
...
/** Définition de la classe historique **/

class Historique{


		/** Fonction qui crée une action, insert dans l'historique
		** - - - - - @param - - - - - - - - - -
		** $idMembre: idMembre (int)
		** $action: message, l'action effectuée (String)
		**/

		public static function creerAction($action) {
			...
			if (isset ($_SESSION['jeton']) ){
				...
				$date =  date('Y-m-d');
				$heure = date('H:i:s');
				$prepstmt = Bdd::connectBdd()->prepare("INSERT INTO  Jejescripthistorique (date,action,ip,id_membre,heure) VALUES (?,?,?,?,?)");
			

				$prepstmt -> bindParam(1,$date);
				$prepstmt -> bindParam(2,$action);
				$prepstmt -> bindParam(3,$ip);
				$prepstmt -> bindParam(4,$idMembre);
				$prepstmt -> bindParam(5,$heure);
		...
		public static function creerActionFis($action) {
			// On vérifie si l'utilisateur est connecté
			if (isset ($_SESSION['jeton']) ){
				$idMembre = $_SESSION['id'];
				$ip = Ip::get_ip();
				$date =  date('Y-m-d');
				$heure = date('H:i:s');
				$prepstmt = Bdd::connectBdd()->prepare("INSERT INTO  Jejescripthistoriquefis (date,action,ip,id_membre,heure) VALUES (?,?,?,?,?)");
		...
		public static function creerActionNonMembre($action) {
			// On vérifie si l'utilisateur est connecté
			if (isset ($_SESSION['jeton']) ){
				$ip = Ip::get_ip();
				$date =  date('Y-m-d');
...
class Inscription {
	...
	public static function inscrire($nom, $prenom, $identifiant, $email, $passeUn, $passeDe) {

		
		if(!empty($nom) AND !empty($prenom) AND !empty($identifiant) AND !empty($email) AND !empty($passeUn) AND !empty($passeDe)) {
				...
				if($verifIdentifiant -> rowCount() != 1) {
					...
					if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
						...
						if($verifMail -> rowCount() != 1) {
							...
							Inscription::profil($nom,$prenom,$identifiant, $email, $passeUn);
							...
							if(isset($_POST['fis'])){echo' <form action="indexFis.php" method="post">
							 <input type="submit" value="Retour à l\' espace personnel" />
							 <input type="hidden" name="page" value="newAccount">
							 </form>';}else {echo' <form action="index.php" method="post">
							 <input type="submit" value="Retour à l\' espace personnel" />
							 <input type="hidden" name="page" value="newAccount">
							 </form>';}
							;	
					...
					if(isset($_POST['fis'])){
						echo' <form action="indexFis.php" method="post">
							 <input type="submit" value="Retour à l\' espace personnel" />
							 <input type="hidden" name="page" value="newAccount">
							 </form>';}
							 else{
	...
	public static function profil($nom, $prenom, $identifiant, $email, $pass) {
	...
	public static function activer($pseudo) {
		...
		switch($methode['id']) {
			...
			if(isset($_POST['fis'])){$resultat = 'Votre inscription est termin&eacute;e, vous pouvez maintenant vous connecter.<br /><a href="'.URLSITE.'/indexFis.php"><span class="blue">Connexion</span></a>';} else {$resultat = 'Votre inscription est termin&eacute;e, vous pouvez maintenant vous connecter.<br /><a href="'.URLSITE.'/index.php"><span class="blue">Connexion</span></a>';}
     ...
     <input type="hidden" name="page" value="Annuler">
			...
			$resultat = 'Votre inscription est termin&eacute;e, un administrateur ou un mod&eacute;rateur doit la valider,<br />un email de confirmation vous sera envoy&eacute; d&egrave;s que cette op&eacute;ration sera effectu&eacute;,<br />pensez a v&eacute;rifier vos spams.';
 echo'<form action="index.php" method="post">
     <input type="submit" value="Retour accueil" />
     <input type="hidden" name="page" value="Annuler">
     </form>';
...
class InscriptionFis {
				...
				if($verifIdentifiant -> rowCount() != 1) {
					...
					if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
						...
						if($verifMail -> rowCount() != 1) {
							...
							InscriptionFis::profil($nom,$prenom,$identifiant, $email, $passeUn);

							InscriptionFis::protect($identifiant);

							InscriptionFis::message($identifiant);

							$resultat = InscriptionFis::activer($identifiant);
	...
	public static function activer($pseudo) {
		...
		switch($methode['id']) {
			...
			Activation::activationAutoFis($pseudo);
			if(isset($_POST['fis'])){$resultat = 'Votre inscription est termin&eacute;e, vous pouvez maintenant vous connecter.<br /><a href="'.URLSITE.'/indexFis.php"><span class="blue">Connexion</span></a>';} else {$resultat = 'Votre inscription est termin&eacute;e, vous pouvez maintenant vous connecter.<br /><a href="'.URLSITE.'/index.php"><span class="blue">Connexion</span></a>';}
	...
	public static function niveau($login) {
		...
		if($donnee['activation'] === '1') {
			...
			switch($donnee['niveau']) {
				...
				$redirect = redirection(URLSITE.'/adminpanel/view/fisStagiaire/index.php');
				...
				$redirect = redirection(URLSITE.'/professeur/index.php');
...
class Activation {
	...
	public static function activationAuto($login) {
		...
		if(isset($_POST['fis'])){
			...
			$niveau='6';
			$resultat2 = Bdd::connectBdd()->prepare(UPDATE.MEMBREZ.NIVEAU.LOGIN);
			$resultat2 -> bindParam(':login', $login);
			$resultat2 -> bindParam(':niveau', $niveau);
			$resultat2 -> execute();
	...
	public static function activationAutoFis($login) {
	...
	// pour activer le nivrau 6 import csv du ministère
	public static function activationAutoCSV($login) {
			$niveau='6';
			$resultat2 = Bdd::connectBdd()->prepare(UPDATE.MEMBREZ.NIVEAU.LOGIN);
			$resultat2 -> bindParam(':login', $login);
			$resultat2 -> bindParam(':niveau', $niveau);
...
class ProtectEspace {
	...
	public static function professeur($id, $captcha, $jeton, $niveau) {
	...
	public static function fis($id, $captcha, $jeton, $niveau) {

		if(empty($id) OR empty($captcha) OR empty($jeton)) {

			redirection(URLSITE.'/deconnexionFis.php');
...
################################################test juillet 2014 ###########################################
	...
	public static function newPass2($id, $newPassUn, $newPassDe) {

		if(!empty($newPassUn) AND !empty($newPassDe)) {
	...
	public static function liste($id) {
		...
		while($donnee = $resultat -> fetch(PDO::FETCH_ASSOC)) {
			...
			if($donnee['lu'] === '1') {
				...
				$image = '<img src="'.URLSITE.'/design/img/email-open.png" width="30" height="33" align="absmiddle">';

			}

			else {

				$image = '<img src="'.URLSITE.'/design/img/email-new-icon.png" width="30" height="33" align="absmiddle">';
	...
	public static function messageAll($titre, $message) {
		...
		if(!empty($titre)) {
			...
			if(!empty($message)) {
				...
				$all -> bindParam(':email', $id, PDO::PARAM_INT, 11);
	...
	public static function listeEnvoi($id) {
		...
		while($donnee = $resultat -> fetch(PDO::FETCH_ASSOC)) {
			...
			if($donnee['effacer']==1) {
				...
				$image = '<img src="'.URLSITE.'/design/img/suppr-icon.png" width="30" height="33" align="absmiddle">';
		...
		while($donnee = $resultat -> fetch(PDO::FETCH_ASSOC)) {
			...
			if($donnee['activation'] === '5') {
				...
				$action = '<input type="hidden" value="'.$donnee['id'].'" name="id"> <input type="submit" name="debannir" value="D&eacute;bannir" class="statut-switch">
				 <input type="submit" name="supprim" value="Supprimer" class="statut-switch">';
			...
			elseif($donnee['activation'] === '0') {
				...
				switch($donnee['niveau']) {
					...
					$action = '<input type="hidden" value="'.$donnee['id'].'" name="id"> <input type="submit" name="bannir" value="Bannir" class="statut-switch"> <input type="submit" name="moderateur" value=" Mod&eacute;rateur" class="statut-switch"> <input type="submit" name="prof" value="Prof" class="statut-switch"><input type="submit" name="supprim" value="Suppr" class="statut-switch">';
					...
					$action = '<input type="hidden" value="'.$donnee['id'].'" name="id"> <input type="submit" name="bannir" value="Bannir" class="statut-switch"> <input type="submit" name="membre" value="Repasser Membre" class="statut-switch"><input type="submit" name="supprim" value="Supprimer" class="statut-switch">';
					...
					$action = '<input type="hidden" value="'.$donnee['id'].'" name="id"> <input type="submit" name="membre" value="Repasser Membre" class="statut-switch"><input type="submit" name="supprim" value="Suppr" class="statut-switch">';
...
class Admin {
	...
	// Passer un membre -> prof
	public static function passeProf($id) {
